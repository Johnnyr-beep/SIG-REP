    ...
    <div className="pila">
      ...
      {data ? (
        <>
          ...
          {tienePermiso("PERMISO_COSTO_POR_PRODUCTO") ? (
            <TablaCostos
  ...
  return (
    <Tarjeta titulo={titulo} sinRelleno>
      {filas.length === 0 ? <Vacio titulo="Sin costo en el corte" detalle="Ninguna línea coincide con los filtros seleccionados." /> : (
        <div className={alta ? "tabla-envoltorio tabla-envoltorio--alta" : "tabla-envoltorio"}><table className={variante ? `tabla tabla--costos tabla--costos--${variante}` : "tabla tabla--costos"}><colgroup><col className="tabla--costos__nombre" /><col className="tabla--costos__importe" /><col className="tabla--costos__importe" /><col className="tabla--costos__importe" /><col className="tabla--costos__importe" /><col className="tabla--costos__porcentaje" /><col className="tabla--costos__importe" /><col className="tabla--costos__porcentaje" /><col className="tabla--costos__porcentaje" /><col className="tabla--costos__lineas" /></colgroup><thead><tr><th scope="col">{etiqueta}</th><th scope="col" className="numero">Venta</th><th scope="col" className="numero">Kilos</th><th scope="col" className="numero">Venta kilo</th><th scope="col" className="numero">Costo</th><th scope="col" className="numero">Costo %</th><th scope="col" className="numero">Margen</th><th scope="col" className="numero">Margen %</th><th scope="col" className="numero">Cobertura</th><th scope="col" className="numero">Líneas</th></tr></thead><tbody>{filas.map((fila) => <tr key={fila.nombre}><th scope="row" title={fila.nombre}>{fila.nombre}</th><td className="numero">{dinero(fila.venta)}</td><td className="numero">{numero(fila.kilos)}</td><td className="numero">{dinero(fila.venta_por_kilo)}</td><td className="numero">{dinero(fila.costo)}</td><td className="numero">{porcentaje(fila.costo_porcentaje)}</td><td className="numero">{dinero(fila.margen_valor)}</td><td className="numero">{porcentaje(fila.margen_porcentaje)}</td><td className="numero">{porcentaje(fila.cobertura_costo)}</td><td className="numero">{numero(fila.lineas_con_costo)} / {numero(fila.lineas)}</td></tr>)}</tbody></table></div>
      )}
    </Tarjeta>
  );
}