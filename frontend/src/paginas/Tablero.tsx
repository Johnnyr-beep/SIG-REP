...
import { Indicador, PieCalculo, Semaforo, notaComparativa } from "@/componentes/indicadores";
import { FORMULAS } from "@/utilidades/dominio";
import { dineroCorto, kilos, porMedida, porcentaje, puntos } from "@/utilidades/formato";
        ...
        <>
          ...
          >
            <div className="consolidado">
              <div className="consolidado__foco">
                ...
                <p className="consolidado__cifra">{porcentaje(data.consolidado.cumplimiento)}</p>
                <Semaforo estado={data.consolidado.semaforo} />
                <p className="consolidado__brecha">
                  {puntos(data.consolidado.brecha)} frente al ideal
                </p>
                <p className="tenue">{notaComparativa(data.consolidado.cumplimiento, data.consolidado.ideal)}</p>
              </div>
              ...
              <Indicador
                ...
                pista={
                  <>
                    ...
                    <p className="tenue">
                      Se calcula sobre los totales, nunca promediando el porcentaje que envía SIESA
                      línea a línea.
                    </p>
                  </>
                }
          ...
          >
            {data.grupos.length === 0 ? (
              <Vacio titulo="Sin grupos" detalle="El período no tiene agrupaciones con presupuesto." />
            ) : (
              <div className="rejilla rejilla--grupos">
                {data.grupos.map((grupo) => (
                  <TarjetaGrupo key={grupo.codigo} grupo={grupo} medida={medida} />
                ))}
              </div>
            )}
          </Tarjeta>
          ...
          >
            {data.sin_presupuesto.length === 0 ? (
              <p className="tenue">
                Ningún punto de venta registró venta fuera del presupuesto en este período.
              </p>
            ) : (
              <ul className="lista-simple">
                {data.sin_presupuesto.map((punto) => (
                  <li key={punto.codigo_co}>
                    <span className="mono">{punto.codigo_co}</span> {punto.nombre}
                    <strong className="empujar">{porMedida(punto.venta, medida)}</strong>
                  </li>
                ))}
              </ul>
            )}
...
function TarjetaGrupo({ grupo, medida }: { grupo: FilaGrupo; medida: "valor" | "kilos" }) {
  return (
    <article className="grupo">
      ...
      <dl className="grupo__detalle">
        ...
        <div>
          ...
          <dd>
            ...
            <span className="tenue">({porcentaje(grupo.cumplimiento_proyectado)})</span>
          </dd>
        </div>
        <div>
      ...
      <Link className="grupo__enlace" to={`/cumplimiento?grupo=${grupo.codigo}&medida=${medida}`}>
